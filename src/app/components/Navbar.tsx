...
const Navbar: React.FC = () => {
  ...
  return (
  <nav className="bg-gray-900 shadow-md fixed w-full top-0 z-10">
      <div className="container mx-auto px-6 py-3 flex justify-between items-center">
  <Link href="/" className="text-2xl font-bold text-white cursor-pointer">
          Amana Bookstore
        </Link>
        <div className="flex items-center space-x-4">
          <Link href="/" className={`text-gray-200 hover:text-green-400 cursor-pointer ${pathname === '/' ? 'text-green-400 font-semibold' : ''}`}>
            Home
          </Link>
          <Link href="/cart" className={`text-gray-200 hover:text-green-400 flex items-center cursor-pointer ${pathname === '/cart' ? 'text-green-400 font-semibold' : ''}`}>
            My Cart
            {cartItemCount > 0 && (
